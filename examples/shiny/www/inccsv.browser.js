var IncCSV = (() => {
  ...
  function parseCsvRecords(text, options = {}) {
    ...
    const comment = options.comment;
    const parsed = import_papaparse.default.parse(text, {
      comments: comment,
      delimiter,
      dynamicTyping: false,
      escapeChar: escape,
  ...
  function writeCsv(columns, rows, options = {}) {
    ...
    return `${import_papaparse.default.unparse(data, {
      delimiter,
      escapeChar: escape,
      header: false,
      newline: "\n",
...
${csvText}`;
  ...
  function writeMetadata(metadata) {
    ...
    for (const section of sectionKeys) {
      assertName(section);
      const sectionKeys2 = Object.keys(metadata[section]).sort();
      if (sectionKeys2.length === 0) {
        fail(`Section [${section}] has no properties.`, "empty_section");
      }
      lines.push(`[${section}]`);
      for (const key of sectionKeys2) {
        assertName(key);
